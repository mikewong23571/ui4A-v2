...
用户随后明确授权通过已登录浏览器代操作两份批准。两份当前均为 **accepted**。首次批准因经典未编辑修订副本与完整候选交接缺陷，先提交后重放失败；D81修复并发布ba146c50后原事件原样重放恢复，未重复批准首份；第二份页面两步批准成功。

新todo:t59已实测编辑备注并完成，完成态仍显示编辑/归档。既有todo-capture:main固定旧bornVersion，仍会在再记一条时带旧值；F10的存量入口问题尚未解决，不因新定义已批准而报全量通过。
