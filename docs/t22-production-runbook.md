...
```bash
pnpm vitest run scripts/t22-image-contract.test.ts scripts/t22-helm-contract.test.ts
```
...
```bash
pnpm --filter @ui4a/worker exec tsx ../../scripts/t22-keycloak-realm-bootstrap.ts --check
pnpm --filter @ui4a/worker exec tsx ../../scripts/t22-keycloak-realm-bootstrap.ts --apply
```
...
`scripts/t22-k8s-replay-drill.ts`；它只使用 bounded fixture，不能替代完整业务验收。
...
pnpm --filter @ui4a/worker exec tsx \
  ../../scripts/t22-k8s-recovery-observe-command.ts capture
...
pnpm --filter @ui4a/worker exec tsx ../../scripts/t22-k8s-recovery-command.ts plan
```
...
```bash
pnpm vitest run scripts/t22-k8s-recovery-live.test.ts \
  scripts/t22-backup-contract.test.ts scripts/t22-restore-contract.test.ts
```
