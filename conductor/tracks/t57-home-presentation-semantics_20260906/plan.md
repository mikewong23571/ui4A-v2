...
- [x] Task: P0.1 复核 T56 与架构基线
  - [x] 读取本目录与正典，核查 T56 实际 plan/commit/review/dirty，登记共用模块交接 SHA。
  - [x] 执行 strict/目录余量与扫描集检查，复核 A1–A6；其他任务脏文件不修改、不提交。
- [x] Task: P0.2 执行 S1 组件姿态与责任保全探针
  - [x] 同对象多 intent、混合集合、unknown/新应用，验证最小词汇/语义扩展与动作可达。
  - [x] 验证 generic/Recipe/Sidecar 的责任保全和授权/依赖失效，记录方案、必要字段与测试路径。
- [x] Task: P0.3 执行 S2 来源真实的直接创建探针
  - [x] 对比既有 client-owned 装配与最小 canonical ingress 扩展，跑来源/重试/id冲突/拒绝/replay。
  - [x] 证明无 LLM human 创建和 CLI/agent 同门，输出 schema、字段所有者及事务/顺序合同。
- [x] Task: P0.4 执行 S3 首页来源与助手同源探针
  - [x] 跑责任/open无委托/历史/空/部分失败/unknown fixtures，核对 HTTP、UI、clientView 和披露。
  - [x] 记录排序/覆盖/分页/请求数量/输入存续，避免给首页创建第二聚合系统或 chat store。
- [x] Task: P0.5 决策与详细设计 review
...
- [x] Task: P1.1 Red 语义选择与扫描边界
  - [x] 钉 US03/10：有动作非责任、无动作可比较、混合成员、unknown、跨域改名的失败测试。
  - [x] 钉 D54 实际文件发现与合法/非法分支的正负例，明确哪些是核心协议机制。
- [x] Task: P1.2 Green 词汇、姿态和动作披露
  - [x] 按 P0 最小方案扩展声明/选择/词汇；低频动作可达，责任卡不退化为隐藏按钮。
  - [x] 同步 catalog/compiler/validator/依赖版本，个人 Sidecar 与共享 Recipe 命中同样正确。
  - [x] 扩展精确 D54 扫描/测试；仅拆本次贴限模块，不新增依赖或例外。
...
- [x] Task: P2.1 Red 首页处境与空/错状态
  - [x] 钉 US01/02/11：首页有 open 线而无 delegation、只有历史、全空、未知、局部失败、无归属责任。
- [x] Task: P2.2 Green 同源首页呈现
  - [x] 扩展 my-work 声明/必要纯读源切片，当前/历史分层；排序有来源，数量/覆盖诚实。
...
- [x] Task: P3.1 Red→Green 创建合同与来源
  - [x] 按 S2 先写 caller/client/来源、幂等、拒绝、身份、CLI 与 replay 失败测试，再实现最小装配。
  - [x] 用户只填目标可直接建线，无模型也可用；真实来源可回读，不丢 required 字段或伪造来源。
- [x] Task: P3.2 Red→Green 动作菜单与表单宿主
  - [x] 钉菜单只选命令、Dialog/非模态场景、键盘/焦点、取消重开、草稿、schema过期与回执。
  - [x] 复用 RJSF/ActionRunner/ActionSubmit，必要薄 primitives；风险文案不虚构 D78.4 保证。
- [x] Task: P3.3 Red→Green 助手入口与跨页
