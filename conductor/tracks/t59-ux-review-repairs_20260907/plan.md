...
## P3 部署与公网验收
- [x] Task: 用户明确授权浏览器完成两份批准；新实例编辑通过，旧出生版本未迁移。
...
- [x] Task: D81 未编辑副本交接 + 事务追加前完整纯应用；真实日志离线重放与负例。
- [x] Task: 发布恢复版本并验证全站读取、首个已提交批准、第二份页面批准。
- [x] Task: 验证新待办定义生效与旧出生版本边界；同步部署记录及最终交付。

## 仍开放
- [ ] Task: F10存量捕捉入口安全升级：当前旧bornVersion仍带旧输入，不能用新定义批准代替迁移验收。
