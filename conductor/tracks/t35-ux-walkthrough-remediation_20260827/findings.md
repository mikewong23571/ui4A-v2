...
- **选择器设计(具体化)**:
  1. **入口**: 材料组 [＋添加涉及对象] → 展开选择器面板(不是 RJSF 表单);
...
- **修复**: ①✅focus 即本线时中栏改协作引导,不重复渲染线面;②✅左轨 w-96 + overflow-x-hidden;③✅chat 在 thread 声明页自动停靠展开(open 联动);④**紧凑密度重设计**(对齐=单列栅格+三栏同基线;对比=目标 H1 最重/状态 chip 次之/动作语义分组+危险色阶;亲密性=现状组/材料组(添加贴清单)/动作语义分组/图例移出内容流)——完整设计见 design-notes §九,恢复执行即实施。
