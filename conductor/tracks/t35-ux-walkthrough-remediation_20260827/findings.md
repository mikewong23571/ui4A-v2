...
**设计方向已定档(后续 track)**: thread 工作台三栏(左线叙述/中共同注视/右 chat,
design-notes §六) · 线内 pin 页面与切线三路径(§七) · inline/委托续跑语义话术
补齐(§八/F-08③)。
...
- **状态**: fixed-pending-recheck(话术与材料清单已落地;对象选择器与清单内一键移出归 W)
...
## F-29 线工作台三栏的重复与布局缺陷(W 阶段自查,浏览器实测)

- **状态**: fixing
- **严重度**: P1
- **发现**: 2026-08-28,W1 上线后现场检视
- **现象**: ① URL focus=thread:X 时,**左轨与中栏渲染同一张线面**——同一叙述并排两份(重复+占地);② 左轨 w-80 内容溢出:钉住的 todos 卡与动作按钮相互压叠、按钮文字被截断;③ chat 进线后仍为悬浮 FAB,未自动停靠为右栏(W1 只实现了"打开时切分栏",未实现"进线即分栏且展开")。
- **修复**: ① focus 即本线时中栏改入口引导(选材料/进应用/与助手协作),不重复渲染线面;② 左轨 w-96 + overflow-x-hidden;③ chat 在 thread 声明页自动停靠展开(open 联动)。
- **证据**: 浏览器现场截图(本轮)。
