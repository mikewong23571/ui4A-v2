...
- **状态**: fixing(布局三项已修;④ 经用户三轮追问**改版定稿**为结构性修复,待实施)
- **严重度**: P1
...
- **修复**: ①✅focus 即本线时中栏改协作引导,不重复渲染线面;②✅左轨 w-96 + overflow-x-hidden(临时止血,④ 落地后由目录化根治);③✅chat 在 thread 声明页自动停靠展开(open 联动);④**结构性修复(§十 定稿)**:pin 语义修正(上下文引用,不再渲染整面)+ 左轨目录化(书桌纯读,hover 露条目操作)+ 布局不变式(**2 轨 + 1 舞台**,轨道只长条目,栏数恒为 3)+ **按钮跟着注视走**(书桌几乎零按钮,舞台是唯一完整动作组,线被注视时呈现推进/归档)——完整定稿见 design-notes **§十**;§九 紧凑密度降级为书桌叙述卡排版配角。
