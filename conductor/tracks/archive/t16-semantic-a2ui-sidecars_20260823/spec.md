...
T16 将渲染定义为独立 Presentation Plane。Chat Agent 只理解一个薄的 Presentation Request/Receipt 协议，负责“是否需要呈现、呈现什么”；独立 Presentation Agent 读取授权合同图并规划 binding-only A2UI Surface Tree。Application 在定义激活后提前枚举典型场景并生成参数化 Render Recipe，运行时再实例化为跨 Session 的用户级 Render Sidecar。Renderer 确定性解引用事实、校验 action、执行布局。Sidecar 是派生展示状态，不进入业务 Entity 字段，不替代业务事件真相。

## Product Thesis

```text
应用合同：事实、关系、动作、约束
Chat Agent：理解目标并发出薄 Presentation Request
Presentation Plane：Application Recipe 预生成、动态规划、User Sidecar fastpath
...
### FR1 Chat/Presentation 薄协议与故障隔离

- Chat Agent 只发出包含 subject、intent、constraints、delivery 和 source message ids 的 Presentation Request，不输出 Surface Tree。
- Chat 只消费 ready/pending/fallback/failed Receipt 与 Sidecar id/version/surface URL；完整 A2UI 内容不进入 Chat prompt/history。
- 显示即时 Preview 不需要人类确认；保存用户 Sidecar、晋升共享 Recipe 和执行业务 action 分属不同事件域。
- Chat answer 与 Presentation planning 独立完成；Presentation 失败不得改变可独立完成的 Chat outcome。
- Direct navigation、Flow transition 与 Chat 使用同一 Broker；生成路径不得由 chat route 业务关键词短路。

### FR2 Application Recipe 预生成

- Application/Flow/Catalog 激活后机械枚举典型 scenario descriptor，并异步调度独立 Presentation Agent。
- Application Recipe 参数化 subject/data slots，不包含 principal、session id、用户偏好或实时事实值。
- Recipe 经 binding/catalog/dependency 校验后进入 candidate registry；生成失败不阻断 Application 激活、Chat 或人工 Renderer。
- candidate 可以作为低优先级机械安全 fastpath；晋升 promoted shared Recipe 必须经 human diff/approval。
- definition/catalog 变化只扫描和重生成受影响 Recipe；不兼容版本立即 stale。

### FR3 Render Situation 与有界合同图
...
### FR6 用户级 Render Sidecar 与 fastpath
...
- Sidecar 只按 principal/policy scope/subject/intent/device 保存用户级 `cache | pinned` 结果；持久化 key/schema 禁止 sessionId。
- Session 只持有 active Sidecar id/version 和未提交 preview/展开态，不拥有独立缓存。
- fastpath lookup 为 user pinned → user cache → promoted Recipe → validated candidate Recipe → generic fallback → runtime Presentation Agent。
...
- User Sidecar 不得影响其他 principal；User Sidecar 命中时 Chat/Presentation LLM 调用均为 0，Application Recipe 命中时 Presentation LLM 调用为 0。
- 用户自然语言或直接操作只修订 User Sidecar；只有经参数化、去用户化和 human approval 才能形成 promoted Application Recipe。
...
- Presentation request/receipt、Recipe、Sidecar、hydration、invalidation 和 promotion 使用独立事件族/Projection；Chat History 仅保存引用。
- Presentation 事件进入同一 append-only 存储时不得改变 Business Snapshot hash。
...
- Application Recipe hit 时 Chat/Presentation 两类 LLM 都不得为呈现额外调用；Chat answer 延迟不得包含异步 Presentation Planner 延迟。
...
- Source governance 证明 Sidecar schema/key 不含 sessionId，Chat prompt/history 不含完整 catalog/Surface/dependency payload。
...
- 实施能力与逐技术故事 DoD：`technical-stories.md`。
- Plane、协议、数据模型、事件与依赖边界：`architecture.md`。
