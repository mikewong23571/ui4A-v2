...
## 问题地图(2026-08-28 R3 收口,全量对账后)

**已关闭(rechecked,27)**: F-01 · F-02 · F-03(S6) · F-04(S6) · F-06 · F-07(F-29④ 危险分层) · F-08(D-3) · F-09(主体;话尾方言=D-4 二期残余) · F-10 · F-11 · F-12 · F-13(收口轮动词统一) · F-14(收口轮修复) · F-15 · F-16 · F-17 · F-18 · F-19 · F-21 · F-22 · F-23 · F-24 · F-25(入口层) · F-27 · F-28 · F-29 · F-30 · F-31。
**wontfix(1)**: F-20(现码不复现,S11 干净种子世界全 UI 序列终判,双证据)。
**open(方向项,不阻塞本 track)**: F-05 术语体系(④ 已随 S7.1 解决,其余为跨面命名方向项)、F-26 入口区(② 阈值收缩/③ 文案标准已达标;剩 ① 应用条 pin)。
...
- **复验记录(2026-08-28,R3)**: S6 全程画布向导面同步可读、同文错误行聚合(≤1)未见堆叠;证据 `evidence/2026-08-28-R3/S6_chat_round*.png`。**状态: rechecked**
...
- **处置**: plan Phase C;复验故事 S6。
- **复验记录(2026-08-28,R3)**: S6 走查目标达成后助手收敛、历史会话成败标注与业务结果一致(发布成功 ≠ 失败);证据 `S6_chat_final.png`、`S6_history.png`。**状态: rechecked**

## F-05 术语体系混乱:一物多名(P1)

- **状态**: open(方向项;④ 流程定义列表 raw id vs 详情中文已随 T35 S7.1 title 投影修复解决——列表/详情均以业务标题为主,2026-08-28)
- **严重度**: P1(业务概念清晰度)
- **发现**: R1(2026-08-27)
...
- **状态**: rechecked(2026-08-28,R3:S1 走查通过——首页簿记数字退场、图例每面一次、self 空卡经 links 过滤退场;证据 `evidence/2026-08-28-R3/S1_home_full.png`、`R3_home.png`)
- **严重度**: P1(重复原则)
- **发现**: R1(2026-08-27)
...
- **状态**: rechecked(2026-08-28:ActionGroup 按合同 requires-confirmation 派生危险组(dashed 分隔+danger 描边),随 F-29④ 落地;R3 实测线舞台 归档/完成 入 danger 层(`F30-4-thread-stage.png`)、首页动作区主次分明)
- **严重度**: P1(对比原则)
- **发现**: R1(2026-08-27)
...
- **状态**: rechecked(2026-08-28:D-3 触发键动作语「X ⌄」全站落地(建线「创建工作线 ⌄」、meta「加动作 ⌄」等);合同层动作 title 中文化(添加涉及对象/挂进本线等);S4/S8 走查通过)
- **严重度**: P1(文案)
...
- **状态**: rechecked(2026-08-28,R3:S6 主体通过——澄清问题业务语言、画布同步可读、会话成败标注一致,证据 `evidence/2026-08-28-R3/S6_chat_*.png`、`S6_history.png`;残余=澄清话尾偶带步骤/必填字段口径一句(D-4 二期,不阻塞)。旁证:S5.4 新会话澄清问句业务语言正常(`S5R-5-chat-first-reply.png`))
- **严重度**: P1(文案/概念)
- **发现**: R1(2026-08-27)
...
- **状态**: rechecked(2026-08-28,R3 收口轮:S7 走查流程详情动作区,废弃禁用原因=「仍有进行中的实例,不能删除该定义(guard 不满足: no-live-instances=false)」人话主句+机器审计括号;证据 `S7R-3-detail-actions.png`;exec 拒绝面机器串保持 D-4 审计层口径)
- **严重度**: P1(文案;诚实机制本身是优点)
- **发现**: R1(2026-08-27)
...
- **状态**: rechecked(2026-08-28,R3:S8 走查通过——线详情叙述页(目标/状态 chip/来源/工作集)+§十书桌/舞台;证据 `evidence/2026-08-28-R3/S8_desk_stage.png`、`S8_desk_with_material.png`、`evidence/2026-08-28-desk/`)
- **严重度**: P1(亲密原则/概念表达)
- **发现**: R1(2026-08-27)
...
- **状态**: rechecked(2026-08-28,R3:S5 走查处境条弹层(站点/scope/注视/跨面桥「查看活实例」「在 meta 中编辑」)一致呈现;证据 `S5R-1-situation-popover.png`;桥接语义随 F-25 入口层重定位收口)
...
- **状态**: rechecked(2026-08-28:受控弹出层落地+动词统一——委托监控按钮「刷新」→「重新载入」(收口轮补齐),canvas/委托监控同词;系统菜单点击项后收起实测 `F19-system-menu.png` 旁证)
- **严重度**: P2
- **发现**: R1(2026-08-27)
...
- **状态**: rechecked(2026-08-28 收口轮修复:EntityView 属性表过滤 presentation(呈现层结构元数据,审计走 RawContractDrawer);测试=entity-view.test 夹具携带 presentation 断言不上表)
- **严重度**: P2
- **发现**: R1(2026-08-27)
...
- **状态**: rechecked(2026-08-28,R3 收口轮:S7 走查 meta 读面全站扫「不代表生产/不冒充/TODO/FIXME」零命中;诊断面板占位目录协议已随呈现链收口)
- **严重度**: P2
- **发现**: R1(2026-08-27)
...
- **状态**: rechecked(2026-08-28,R3 收口轮:首页导航 aria-current 当前项=「我的事」实测可见;前缀映射派生,零特判)
- **严重度**: P1(对比/定位)
...
- **状态**: rechecked(2026-08-28,R3 收口轮:受控弹出层点击菜单项后收起+点击外部关闭实测(`F19-system-menu.png`);样式随 D-7 基座)
- **严重度**: P2
...
- **状态**: wontfix(2026-08-28 终判:现码不复现,双证据;见复验记录)
- **严重度**: P1(产品内治理路径的正确性)
...
- **终判记录(2026-08-28,R3 收口轮 S11 全 UI 序列,干净种子世界)**: **wontfix(不复现),双证据**——
  1. **正向**:todo-item 修订(open 节点加 pin/置顶→open)全 UI 链路:修订→definition-revised → 加动作(结构化表单)→definition-edited → 提交校验→definition-submitted(九项 checks 全过)→ 激活队列 a1 → 人类批准 → v2 active(v1 superseded);**v2 定义 open 节点含 pin 且携带派生 effect `[{type:transition,to:open}]`**——动作未丢、效应在场;工作站生效(open todo item 动作区实测出现「置顶」)。证据 `S11R-1..13`(含事件 207-212 链)。
  2. **反向(旧现象之所以可能)**:复刻原始 payload(archived 加 restore→open)→ 提交校验被 **terminal-reachable 诚实拒绝**(「流程没有任何 terminal 节点」)——restore→open 消灭唯一终态;旧世界该 payload 能"激活成功但节点为空",恰因丢动作 bug 掩盖了非法拓扑;现码两道闸(效应派生+不变式校验)都在。
  - 机制注记:flow 生命周期草稿不进 meta/drafts(T17 Draft 域=agent 候选/CLI 提交,另一机制);生命周期草稿可见性=meta/flows 列表 draft 徽标+事件审计,S11 判定按此口径通过。

## F-21 集合成员状态显示机器节点名而非节点标题(P1)

- **状态**: rechecked(2026-08-28,R3:S9/S10 走查成员状态显示节点标题(进行中/已捕捉/发展中/已归档);证据 `evidence/2026-08-28-R3/S9_recorded_loop.png`、`S10_archived.png`、收口轮线详情「进行中/已归档」chip 同口径(`F30-4/7`))
- **严重度**: P1(任务语言,F-05 家族)
- **发现**: R2(2026-08-28)
...
- **状态**: rechecked(2026-08-28,R3:S6 走查分栏态聊天全程输入框在屏(轮次往返+历史会话);证据 `evidence/2026-08-28-R3/S6_chat_round1.png`~`S6_history.png`;sticky 视口列 + dvh 生效)
...
- **状态**: rechecked(2026-08-28,R3 收口轮:首页书架层「应用(7 个)」实测——内容发布/社区互动/软件实施/编辑写作/Agent 治理/待办事项 + 更多应用(1)(ideas 折叠),点入即该应用 entry;证据 `R3_home.png`、`S5R-3-home-after-jump.png`)
...
- **状态**: rechecked(2026-08-28,R3:首页无恒亮蓝框、0 计数退场为引导句;证据 `evidence/2026-08-28-R3/R3_home.png`;收口轮旁证 `F30-*` 首屏零蓝框零裸数字)
- **严重度**: P1(视觉层级,用户反馈)
...
- **状态**: rechecked(2026-08-28,R3 收口轮:/canvas 无 focus = 注视引导 + 应用/集合入口层,不再默认 articles;证据 `F25-canvas-no-focus.png`)
...
- **状态**: open(部分落地:② 阈值收缩已实测(7 应用 > 6 → 「更多应用(1)」折叠)、③ 数词+名词文案达标(「应用(7 个)」);**剩余=① 应用条 pin**(钉住常用应用常驻前排,复用 sidecar/pin 机制)未实现,保留为方向项随下轮呈现迭代)
...
- **状态**: rechecked(2026-08-28,R3:S8 + desk 走查——「＋ 添加涉及对象」选择器(四集合分组/业务标题+状态/已在本线禁选/点击即挂)、「挂进本线/已挂进本线」pin 语、材料清单与价值句、detach 经 hover 移出合同;证据 `evidence/2026-08-28-desk/`、`S8_desk_with_material.png`、`F30-4-thread-stage.png`)
...
- **状态**: rechecked(2026-08-28,R3:S8 全量复验通过——2 轨+1 舞台形态、书桌纯读目录、pin=上下文条目、按钮跟着注视走(舞台 暂停/完成/归档 + 书桌仅「＋添加涉及对象」)、危险分层;证据 `evidence/2026-08-28-R3/S8_desk_stage.png`、`F30-4-thread-stage.png`(归档/完成入 danger 层))
- **严重度**: P1
...
- **复验记录(2026-08-28,浏览器实测)**: 书桌渲染/选择器四集合分组(articles/todos/ideas/评论,业务标题+状态+已在本线禁选)/点击即挂(context)/条目点击唤起注视/去重(钉住+context 单条目)/hover 移出走合同 detach/舞台暂停→书桌状态 chip 即时翻转为已暂停/恢复;截图 `evidence/2026-08-28-desk/`(desk-initial/desk-selector/gaze-with-pin)。组件测试 6 项 + 组件套件 268 项全绿。
- **证据**: 浏览器现场截图(本轮);`evidence/2026-08-28-W/`。

## F-30 工作线归档无确认门(P2)

- **状态**: rechecked(2026-08-28,R3 收口轮浏览器实测:点「归档工作线」→ 两段式「已请求'归档工作线',尚未执行。/确认并执行/取消请求」;确认后线 → 已归档;human 高风险走 UI 两段确认,agent 提议走引擎挂起(与 post 归档同规);证据 `F30-5-archive-clicked.png`、`F30-6-confirm-stage.png`、`F30-7-after-confirm.png`)
- **严重度**: P2
...
**已通过(全部 11 个故事)**: S1 · S2(2026-08-28 收口轮四项全过,F-31 rechecked) · S3 · S4 · S5(收口轮:落点=真实实例集合/处境条一致/聊天新会话不污染,`S5R-*`) · S6(主体;话尾方言=D-4 二期残余) · S7(收口轮四项全过:title 投影+列表/详情同名+动作区人话禁用原因+零开发注释;`S7R-1..3`) · S8 · S9 · S10 · S11(收口轮:修订→加动作→提交校验→人类批准→v2 激活→工作站「置顶」生效,全链路留痕;F-20 终判 wontfix 双证据;`S11R-1..13`)。
**不通过**: (无)。
...
**收口前置(Phase F)**: ~~F-31 修复~~(✅ S2 四项全过)→ ~~S5/S7/S11 走查~~(✅ 2026-08-28 全过,F-20 终判 wontfix)→ ~~findings 全量对账~~(✅ 2026-08-28 全量回填,27 rechecked + 1 wontfix + 2 方向项 open)→ `CI=true pnpm e2e invariants` 复跑 → DONE 报告 → plan.md F1/F3/F4 勾选、track 关闭。
