...
- [ ] Task: CLI application-bundle 写路径
  - [ ] 红测:apps/cli 单测(--kind application-bundle 显式 lens);e2e/cli-meta-drafts.spec.ts 增例(真实 server)
  - [ ] 绿:实现
- [ ] Task: Chat 同门故事(US6)
  - [ ] 注入驱动协议测试:Assistant 经 meta exec 合同提交 application-bundle 提案(同一裁决路径);可选真实 LLM eval(pnpm eval:llm 口径)
- [ ] Task: Playwright Golden Story(US1–US3 浏览器门)
...
- [ ] Task: Track 收口(archive、registry 状态更新)
