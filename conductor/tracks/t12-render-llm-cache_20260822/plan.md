...
- [~] Task: 路由 fallthrough(rule miss → buildRenderPrompt[词汇表+sitemap 处境] → LLM → parseRenderResponse → validateSpec → freezeSpec;失败交回普通循环;无 key 跳过保 I1)(TDD:mock LLM;非法 JSON/零字面违规/假字段三拒)
