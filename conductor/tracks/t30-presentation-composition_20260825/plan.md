...
> Phase B–F 的具体形状以 Phase A 的 DECISIONS 条目(D45)为准;若 spike 结论改变任务
> 内容,先按 Task Correction 流程修订本计划再施工。
...
- [ ] Task: spike 五问决断,落 DECISIONS.md D45
  - 按 spec.md Phase 0 五问逐一给出:候选、约束、采纳、否决项与理由;核心抉择:
    ...
    复用 vs 新 region 节点)与单主体退化形态(统一包区域层 vs 省略 wrapper)、
    recipe slot/promotion 一般化粒度、credentialed 逐源降级形状与 reasonCode 词汇
  - 可用纯函数原型验证组合规划/退化等价语义(不提交或提交为 spike 测试);
    锚点:spec.md"现状事实"节
  - 验收:DECISIONS.md 新增条目,编号顺延(当前最新 D44);五问各有明确采纳/否决;
    否决项写明理由(防复辟)
- [ ] Task: spec/plan 回改对齐 D45
  - 若 spike 结论改变"最终形态"表述或后续 Phase 任务形状,同步修订 spec.md 与
    本 plan;tracks.md 无需动(本 Track 已登记)
- [ ] Task: Phase Verification & Checkpoint(Refer to workflow.md)

## Phase B: 组合声明模型(数据 + 严格 parse)

- [ ] Task: CompositionDeclaration 类型与严格 parse
  - 归属按 D45(packages/shared 或 packages/engine):声明类型(id/version/
    regions[{region, source rel, intent, mode?}])、虚主体 id 规则(参照 D44
    thread id 口径 1–64 字符 `[a-z0-9][a-z0-9._-]*`)、字段白名单与有界(区域数
    与字符串长上限,参照 `MAX_DATA_LENS_SELECTORS=32` /
    `MAX_PRESENCE_VALUE_LENGTH` 口径)
  - wire 形状接线按 D45(`parseRenderSubject`/`parsePresentationRequest` 扩展或
    保留字前缀解析,`packages/shared/src/presentation/presentation.ts:303-364`);
    既有禁键清单(surface/component/bind/dependency/sessionId)不变
  - Red:非法声明/未知字段/超界/非法 id/未知源 rel 形态一律拒绝的解析测试
- [ ] Task: 内建声明注册表与 my-work 首声明
  - 注册表 = 纯数据 + 查找(版本化,零分支);注册 `my-work`:在等我(`inbox`)、
    在动(`delegations`)、工作线(`threads`)——区域源全部为 sitemap 可达合同
    实体,区域 intent 取自 scenario 既有词汇或按 D45 新增
...
- [ ] Task: 区域组装 planner
  ...
  - Red→Green:声明 → 树形状/binding 完整性(零字面量,I2 口径)/依赖并集覆盖
    全部聚合源 + 声明版本的断言
- [ ] Task: 单主体退化统一
  - 单主体 surface 经同一组合路径产出(单区域退化,形状以 D45 为准);
    validate/normalize/hash 覆盖组合形状(新校验逻辑进新模块);compiler
    component id 约定(`root`/`node:<id>`)稳定或一次性迁移
...
- [ ] Task: key 与 recipe slot 一般化
  - `UserSidecarKey.subject` 承载虚主体标识(形状以 D45 为准;
    `sidecarKeyFingerprint` 规范化覆盖);`ApplicationRecipeKey.subjectShape` 承载
    组合形状;slots 按区域一般化('collection' slot kind 首次产出);
    `promotion.ts:108/119` 与 `apps/web/src/engine/presentation/runtime.ts:141-156`
    的单 slot 硬编码同步一般化(按 slot 形状匹配,不再按数量==1)
...
- [ ] Task: broker 授权与运行时接线
  - broker authorize 对组合主体解析声明并逐区域源 fresh getEntity 重授权;
    credentialed 逐源按 policy scope 降级(形状与 reasonCode 以 D45 为准,收据
    诚实);生命周期事件(presentation-requested/resolved/failed)沿用;
...
- [ ] Task: my-work 端到端 proof
  - `POST /api/presentation`(subject = my-work 虚主体)→ receipt ready →
...
2. 组合声明为数据:parse 严格、注册表零分支;my-work 三区域全部来自 sitemap
   可达合同实体,零每区域/每实体类型特判;
3. 组合与单主体同一台机器(planner/recipe/sidecar/deref/compiler 零分叉),
   单主体 = 单区域退化;binding-only 与薄 chat 边界不变;
4. sidecar 组合主体同生命周期(pin/stale/rehydrate/promote);依赖并集任一源
   失效按声明 mode 触发;命中逐源重授权,credentialed 降级不泄漏;
