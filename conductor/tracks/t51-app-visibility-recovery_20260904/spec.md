...
授予集合、授权模式、治理展开来源标注),与 CLI `auth status` 同事实源。
...
- **D70.1 激活披露口径**:meta exec 的 approve 成功响应可携带 `disclosure` 字段,
  ...
  三分支 `immediately-visible | visible-after-relogin | requires-idp-grant`。披露是
  给批准者本人的表现层数据,**不落事件日志、不进入跨 principal 可见面**(不泄露
  存在性);发现文档过滤语义零变化(D51 不动)。
...
- **US2 我的授权面板**(F2):`/api/auth/session` 返回授权投影;顶栏系统区入口;
  治理展开来源标注;只读;production-profile 路由测试覆盖 200/401。
