...
- **业务优先原则(2026-08-26 授权;D53 修订)**:治理的行数上限(check-size)
  不驱动代码裁剪——不为凑行数裁剪功能或验收证据,文件按架构归属落位。
  超限的处置是**变更时沿功能边界拆解**(D53"膨胀即拆解"纪律);例外登记
  逃生门已随 strict 接线退役(size-baseline 与 dependencyExceptions 均空,
  写入须先修订 DECISIONS.md D53)。派发 subagent 的 prompt 必须写明该策略
  (以及相关 leaf 目录的现值/余量事实),subagent 遇治理失败**只如实报告,
  不自行核算预算、不裁剪代码**;
