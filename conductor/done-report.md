...
> 生成:2026-08-21(自治编排,T1–T8 全部完成)。对照 GOAL.md 的 DONE 定义逐条给出证据。
> 复验命令:`CI=true pnpm check`(821 单测)→ `CI=true pnpm e2e`(42 过 + 1 门控跳过);不变量套件可单跑 `CI=true pnpm e2e invariants`。
...
|---|---|---|---|
| I1 | 撤销全部 LLM key:B1–B3、表单版 S1、哑渲染仍全过 | ✅ | `I1 零智能完整 › 显式撤销 GLM_API_KEY…`(进程 env 断言 + B1–B3 + RJSF 版 S1 + 态势/事件流哑渲染) |
...
- 五条铁律:评审(T2)逐条核对符合;I1/I2/I3/I4 + 审计渲染零 AI(diff/事件流路径断言)。
...
测试总量:821 单测 + 43 E2E(42 过 + 1 真实 LLM 门控);命令:`CI=true pnpm check && CI=true pnpm e2e`。
