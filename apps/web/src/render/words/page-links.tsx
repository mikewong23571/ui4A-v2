 ...
 * 回退 focus 落点(既有行为)。过滤/翻页是读面导航机械:只改查询状态,永不
 * exec、零业务事件;参数语义与 /api/entity 请求同形(人机同门,agent 走同一
 * next/prev 链接)。
 */
import {
...
export function PageLinksWord(props: WordProps) {
  ...
  const navigateToDeclaredPage = (href: string): void => {
    const target = collectionQueryFromContractHref(href);
    // 无 rel 可导航的声明链接诚实不动(不发明任何推算)。
    if (target === undefined) return;
    if (hostNavigate !== undefined) {
      hostNavigate({ offset: target.query.offset, filter: target.query.filter });
      return;
    }
