 ...
 * markdown/detail。Phase B(本版):component 接真实词条组件(words/,
 * lazy 引用——调用返回 Promise<ComponentType>,目录可序列化);bindSchema
 * 收紧为词条级形状约束(Ajv 消费,word-bind.ts):引用节点类型
 * (ref/field/collection+dimension)与必需键逐词条声明。
 */
...
export interface RenderWordEntry {
  ...
  description: string;
  /**
   * 词条组件的 lazy 引用(动态 import:目录端点引入本模块时不执行,
   * 客户端渲染时才加载词条实现;约定:调用返回 Promise<ComponentType>)。
   */
  component: () => Promise<ComponentType<WordProps>>;
...
// ---- 引用节点的 JSON Schema 子形状(目录与校验同源;validator.ts 语义)--

/** 实体引用节点:`{ref: "entity:<rel>"}`。 */
const refNode = {
  type: 'object',
  required: ['ref'],
  properties: { ref: { type: 'string', pattern: '^entity:.+' } },
  additionalProperties: false,
} as const;

/** 字段引用节点:`{field: "<rel>.<path>"}`。 */
const fieldNode = {
  type: 'object',
  required: ['field'],
  properties: { field: { type: 'string', pattern: '^[^.]+\\..+$' } },
  additionalProperties: false,
} as const;
...
const tableWord: RenderWordEntry = {
  name: 'table',
  title: '表格',
  description: '集合数据的表格视图:rows 绑定集合引用(collection);可选 caption 为字段引用',
  component: () => import('./words/table').then((module) => module.TableWord),
  supportsDimension: false,
  bindSchema: {
    type: 'object',
    description: '表格词条:rows 为集合引用(collection);可选 caption 为字段引用',
    properties: { rows: collectionNode, caption: fieldNode },
    required: ['rows'],
    additionalProperties: false,
  },
};

/** 词条:图表(Recharts[shadcn 未初始化,偏差 D13];series = collection+dimension 聚合)。 */
const chartWord: RenderWordEntry = {
  name: 'chart',
  title: '图表',
  description: '聚合图表:series 绑定集合引用 + dimension 维度声明,解引用器分组计数',
  component: () => import('./words/chart').then((module) => module.ChartWord),
  supportsDimension: true,
  bindSchema: {
    type: 'object',
    description: '图表词条:series 为集合引用(带 dimension 维度声明)→ [{key,count}]',
    properties: { series: dimensionedCollectionNode, caption: fieldNode },
    required: ['series'],
    additionalProperties: false,
  },
};
...
const statWord: RenderWordEntry = {
  ...
  component: () => import('./words/stat').then((module) => module.StatWord),
  supportsDimension: false,
  bindSchema: {
    type: 'object',
    description: '统计卡词条:value 为字段引用;label 为字段引用(标题)',
    properties: { value: fieldNode, label: fieldNode },
    required: ['value'],
    additionalProperties: false,
  },
};
...
const timelineWord: RenderWordEntry = {
  ...
  component: () => import('./words/timeline').then((module) => module.TimelineWord),
  supportsDimension: false,
  bindSchema: {
    type: 'object',
    description: '时间线词条:events 为集合引用(append 序即时间序)',
    properties: { events: collectionNode, caption: fieldNode },
    required: ['events'],
    additionalProperties: false,
  },
};
...
const flowWord: RenderWordEntry = {
  ...
  component: () => import('./words/flow').then((module) => module.FlowWord),
  supportsDimension: false,
  bindSchema: {
    type: 'object',
    description: '流程图词条:graph 为实体引用(节点/边数据来自实体)',
    properties: { graph: refNode },
    required: ['graph'],
    additionalProperties: false,
  },
};
...
const formWord: RenderWordEntry = {
  ...
  component: () => import('./words/form').then((module) => module.FormWord),
  supportsDimension: false,
  bindSchema: {
    type: 'object',
    description: '表单词条:entity 为实体引用(动作字段 schema 即输入)',
    properties: { entity: refNode },
    required: ['entity'],
    additionalProperties: false,
  },
};
...
const diffWord: RenderWordEntry = {
  ...
  component: () => import('./words/diff').then((module) => module.DiffWord),
  supportsDimension: false,
  bindSchema: {
    type: 'object',
...
const kanbanWord: RenderWordEntry = {
  ...
  component: () => import('./words/kanban').then((module) => module.KanbanWord),
  supportsDimension: false,
  bindSchema: {
    type: 'object',
    description: '看板词条:columns 为集合引用(成员为卡片)',
    properties: { columns: collectionNode },
    required: ['columns'],
    additionalProperties: false,
  },
};
...
const markdownWord: RenderWordEntry = {
  ...
  component: () => import('./words/markdown').then((module) => module.MarkdownWord),
  supportsDimension: false,
  bindSchema: {
    type: 'object',
...
/** 词条:详情(极简卡片[shadcn 未初始化,偏差 D13];entity = 实体引用,四件组装直出)。 */
const detailWord: RenderWordEntry = {
  name: 'detail',
  title: '详情',
  description: '实体详情:entity 绑定实体引用,properties/actions/links 四件组装直出',
  component: () => import('./words/detail').then((module) => module.DetailWord),
  supportsDimension: false,
  bindSchema: {
    type: 'object',
