 ...
 */
import { resolveLlmConfig, llmRoutingHeaders, type AgentGoal } from '@ui4a/agent';
  ...
  try {
    const response = await fetchImpl(`${config.baseURL}/chat/completions`, {
      ...
      headers: {
        ...
        ...(await llmRoutingHeaders(config.sessionHeader, crypto.randomUUID())),
      },
      body: JSON.stringify({
        model: config.model,
        messages: [
