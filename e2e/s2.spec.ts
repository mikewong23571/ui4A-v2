 ...
 * ② 修正(to: done)→ submit → checks 六项过 → pending-approval(activation
...
test('S2 主链路:非法定义拒且留痕 → 修正 → submit/pending(diff+checks)→ BIOS 人类批准 → sitemap bump → 在途 v1 undeclared → agent 零 prompt 用新动作 pin', async ({
  ...
  await withFreshServer(async () => {
    ...
    // ---- ② 修正:to: done → 通过;submit → checks 六项过 → pending-approval ----
    ...
    // activation 实体:pending-approval + checks 六项全过 + 机械 diff(纯数据)
