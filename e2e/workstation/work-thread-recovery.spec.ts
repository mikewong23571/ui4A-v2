 ...
 *
 * - **空线(Fixture C)**:无材料/active/approval/event,goalSource 不可解析
 *   → 来源行干净省略(裸标识只在 raw 层);空态引导走声明 emptyMeaning,
...
test.describe('work-thread recovery', () => {
  test('US10 空线与不可读对象:空态引导非说明书,来源省略,缺失对象中性空态有恢复出口', async ({
    page,
  }) => {
    ...
    await withFreshServer(async () => {
      ...
      await page.request.post(`${SCENARIO_BASE}/api/exec`, {
        data: {
          rel: 'threads',
          action: 'create',
          params: { id: thread, goal: '空线起步走查', goalSource: `chat:${thread}-missing` },
          actor: 'human',
      ...
      // 目标为主内容;无成员卡;来源不可解析 → 干净省略(F-08)。
      await expect(main.getByText('空线起步走查')).toBeVisible();
      await expect(page.locator('[data-word="member-card"]')).toHaveCount(0);
    ...
    await withFreshServer(async () => {
      ...
      await page.request.post(`${SCENARIO_BASE}/api/exec`, {
        data: {
          rel: 'threads',
          action: 'create',
          params: { id: secret, goal: secretGoal, goalSource: 'e2e:secret' },
          actor: 'human',
      ...
      await expect(caseyPage.locator('[data-word="member-card"]')).toHaveCount(0);
