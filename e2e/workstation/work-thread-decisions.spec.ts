...
test.describe('work-thread decisions', () => {
  ...
  test('US03 active 目标状态变化后,概览经导航回线即更新(成员卡状态指针与「上次停在哪」随合同变化)', async ({
    ...
    await withFreshServer(async () => {
      ...
      // 本线成员全部无声明动作 → generic 规划选 entity-link 词(member-card 词
      // 仅在成员集声明动作时启用,见 surface/generic.ts membersDeclareActions);
      // 以 href 锚定成员(推进后声明 title 会成为身份行,rel 不变)。
      const activeCard = page.locator('[data-word="entity-link"][href*="article-drafting"]');
      ...
      await expect(
        page.locator('[data-word="entity-link"][href*="article-drafting"]'),
      ).toContainText('classification', { timeout: 15_000 });
