...
## D83 工作上下文主次呈现与材料选择（2026-09-08，T61）

- 用户确认：工作内容占主位，材料按需打开，文案只保留状态、事实、依据与动作。原型中的示例阶段和模拟执行不得成为生产分支。
- 增加通用认知 trait `work-context` 与 `supporting-context`：前者声明围绕目标的工作组，后者声明参考上下文；工作线与其 context 引用分别产生该声明。active/approval 的真实责任语义优先，任何材料披露不得隐藏人类责任。
- 通用 `work-content` 呈现词消费已授权 entities/actions/links 绑定，主区呈现工作成员，支持上下文收为按需入口；材料与生命周期管理分开。无该目录词时仍按完整合同呈现。新目录版本使已保存的旧呈现重新规划。
- 工作对象按规范引用重新授权读取，正文只使用声明字段；查看材料和查看既有成果不改变工作状态。输入范围、材料成员、本轮实际读取仍是三个不同事实，不造阶段摘要或第二状态库。
- 选择器采用查看/选择/提交分离、规范引用去重、声明来源消歧与逐项真实回执；不把连续 attach 假称原子事务，不重放已成功项。界面提交继续经过现有 fresh-read 与引擎裁决。
