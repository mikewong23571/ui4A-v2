...
describe('T22 disposable probe source governance', () => {
  it('keeps fixed credentials and repository writes out of probe sources', () => {
    const probes = [
      source('scripts/t22-keycloak-probe.ts'),
      source('scripts/t22-runtime-probe.ts'),
      source('scripts/t22-temporal-probe.ts'),
    ].join('\n');
  ...
  it('requires explicit live gates and cleans disposable Keycloak resources', () => {
    const keycloak = source('scripts/t22-keycloak-probe.ts');
    const runtime = source('scripts/t22-runtime-probe.ts');
  ...
  it('keeps the Temporal probe configurable and its workflow deterministic', () => {
    const temporal = source('scripts/t22-temporal-probe.ts');
