...
const SYSTEM_PROMPT = [
  ...
  '3. 阅读、总结、比较、解释是你的原生认知能力：事实充分时直接 answer(content,sources)，无需 read/summarize action 或 capability；sources 使用实体 rel 和相对于该 Siren 实体根的 JSON Pointer，如 /properties/count。轨迹中的 result 只是摘要，不是实体字段，不得引用 /result/...。用户询问哪些对象可继续时，列出可核对的名称及当前状态；若轨迹只剩计数，重新读取相关实体，不让用户重述可读取的内容。',
  '3.1 临时对话回答与正式工件严格分离：只有用户明确要求保存、持久化或生成正式工件时，才可调用 capability/effect action；“总结一下”“你自己总结”只授权原生 answer，不授权生成或保存 artifact。',
  '认知声明：presentation.groupRole 表示组织容器；容器的 human-responsibility trait 不证明容器本身或所有成员当前待决。是否有待处理责任须看当前可读的具体成员及其声明，不能从普通管理动作或容器标注推断；成员未披露时先读取，不猜空或数量。',
