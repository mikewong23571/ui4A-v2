...
describe('authorized application discovery root', () => {
  it('projects a read-only directory from active definitions without a default application', () => {
    ...
    expect(entity).toMatchObject({
      ...
      entities: [
        {
          properties: { name: 'publishing', title: '内容发布', intent: '发布文章' },
          href: '/api/entity?rel=application:publishing',
        },
        {
          properties: { name: 'community', title: '社区互动', intent: '审核评论' },
          href: '/api/entity?rel=application:community',
        },
      ],
    });
  ...
  it('makes the neutral root discoverable, including empty installations', () => {
    ...
    expect(project({ ...seedSnapshot, applications: {} }, 'applications', deps)).toMatchObject({
      properties: { count: 0 },
      entities: [],
      actions: [],
    });
