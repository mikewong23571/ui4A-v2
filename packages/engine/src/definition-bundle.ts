...
export function exportDefinitionBundle(
  ...
  const flows = entries.map((entry) => activeDefinitionOf(snapshot, entry.name)!).map(parseFlowDefinition);
  ...
  const capabilities = [...capabilityNames].map((name) => snapshot.capabilities![name]!).map(parseCapabilityDefinition);
  return {
    schema: DEFINITION_BUNDLE_SCHEMA,
    bundle: { name: applicationName, version: Math.max(1, ...entries.map((entry) => entry.version)) },
