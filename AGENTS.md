...
- `src/engine/drafts/`: Siren Draft/activation projection, validation/diff adapters, and human-only atomic activation. `application-bundle.ts` owns the genesis kind's validation/inventory-diff/checks plus the D71.5 name-burn predicates (active ∪ deprecated); `activate-flow/agent/application` are the per-kind atomic activation planners (bootstrap-kind seed events for genesis); `draft-action-schemas.ts` assembles create/revise action schemas with the D69 `x-ui4a-payload-schemas` annotation.
...
- `src/chat/`: chat-session start, SSE streaming, history, trail, and decision projection. Chat is an event-log projection and an agent entry point, not an alternate command path. `message-entity.ts` exposes the principal-scoped read-only `message:` projection consumed by `/api/entity` (T54/G14; messages are work material, never executable).
- `src/auth/`: production request identity, browser session lifecycle, and the D70 auth-plane surfaces — activation visibility disclosure (`activation-disclosure.ts`, attached to approve exec responses), `GET /api/auth/session` (read-only authorization projection behind the「我的授权」panel), and auth-plane controls (「刷新授权」re-runs `/auth/login`). Authorization inputs stay credential-derived (D51/D66.4); the panel is a projection, never a second authority.
